...
namespace RefrienderCore {
	public abstract class SpanCompression : ICompressionAlgo {
		...
		public byte[] Decompress(byte[] data, int offset, int compressedSize, int decompressedSize) {
			...
			Decompress(new ReadOnlySpan<byte>(data, offset, compressedSize), arr);
			return arr;
		}
		...
		public int TryDecompress(byte[] data, int offset, int inputSize, int maxLen) {
			...
			return Decompress(new ReadOnlySpan<byte>(data, offset, inputSize), Buffer.Value);
		}

		protected abstract int Decompress(ReadOnlySpan<byte> input, Span<byte> output);
	}
}