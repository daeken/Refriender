...
Dictionary<string, CompressionAlgorithm> Algorithms = new() {
...
var RevAlgorithms = Algorithms.ToDictionary(x => x.Value, x => x.Key);

void DisplayHelp(ParserResult<Options> result) {
	Console.Error.WriteLine(HelpText.AutoBuild(result, h => {
		h.AdditionalNewLineAfterOption = false;
		h.Heading = $"Refriender {typeof(Options).Assembly.GetName().Version}";
